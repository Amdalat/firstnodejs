...
// SIGNUP - Add a new user to the database
const signUp = async (req, res) => {
    // Assuming 'Auth' model and bcrypt for password hashing are available
...
// SIGNIN - Sign in a user and return a token
const signIn = async (req, res) => {
    // Assuming 'Auth' model and jwt for token generation are available
    ...
    }    const token = jwt.sign({ id: user._id }, 'secret'); // Replace 'secret' with a real secret
...
const verifyToken = (req, res, next) => {
  const token = req.cookies.access_token; // Attempt to get the token from cookies
